...
if (process.env.TELEGRAM_BOT_TOKEN) {
    ...
    console.warn("TELEGRAM_BOT_TOKEN is missing. Telegram notifications will be disabled.");
}
...
module.exports.submitContactForm = catchAsync(async (req, res) => {
    ...
    // Notify Telegram asynchronously (don't await)
    notifyNewContact(newContact);

    // Send a JSON response for the AJAX request
    res.status(200).json({ message: 'Thanks for reaching out! We received your message.' });
});

/**
 * Helper to send Telegram notification for new contact
 * @param {Object} contactData - The contact document
 */
const notifyNewContact = async (contactData) => {
    try {
        if (bot && process.env.TELEGRAM_CHAT_ID) {
            let telegramMessage = `New Contact Message!\n\nName: ${contactData.name}\nEmail: ${contactData.email}\nPhone: ${contactData.phone || 'N/A'}\nMessage: ${contactData.message}`;
            if (contactData.attachment && contactData.attachment.url) {
                telegramMessage += `\nAttachment: ${contactData.attachment.url}`;
            }
...
module.exports.viewContacts = catchAsync(async (req, res) => {
...
module.exports.viewSingleContact = catchAsync(async (req, res) => {
    const { id } = req.params;
...
module.exports.deleteContact = catchAsync(async (req, res) => {
...
// Renders the form to add a new contact entry (Admin).
module.exports.renderAddContact = (req, res) => {
    res.render('inputs/updatingContact', { action: 'add' });
};

// Processes the submission of a new contact form (Admin) and saves it to the database.
module.exports.addContact = catchAsync(async (req, res) => {
    const newContact = new contact(req.body);
    await newContact.save(); // Save the new contact details.
    if (req.body.save_action === 'save_and_add_another') {
        return res.redirect('/updating/contact/add');
    }
    res.redirect('/view-contacts'); // Redirect to view all contacts.
});

// Deletes multiple contact messages and their associated attachments.
module.exports.deleteBatchContact = catchAsync(async (req, res) => {
    const { ids } = req.body;
    if (ids && ids.length > 0) {
        // Find contacts to get their attachment filenames
        const contactsToDelete = await contact.find({ _id: { $in: ids } });
        
        for (const contactItem of contactsToDelete) {
            if (contactItem.attachment && contactItem.attachment.filename) {
                await supabase.storage
                    .from(process.env.SUPABASE_BUCKET)
                    .remove([`attachments/${contactItem.attachment.filename}`]);
            }
        }
...
module.exports.renderDeleteContact = catchAsync(async (req, res) => {
...
// We're making our custom file upload error handler available for use in our routes.
module.exports.renderEditContactForm = catchAsync(async (req, res) => {
    const { id } = req.params;
    const foundContact = await contact.findById(id);
    if (!foundContact) {
...
module.exports.updateContact = catchAsync(async (req, res) => {
